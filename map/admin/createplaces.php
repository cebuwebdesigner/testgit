...
if( $_FILES["file"]["name"]!= ""){
	...
	if (file_exists("../temp/" . $_FILES["file"]["name"]))
      {
      echo 'WARNING:'.$_FILES["file"]["name"] . " already exists. (please change the filename and add again...)<br />";
	  }
    else
      {
move_uploaded_file($_FILES["file"]["tmp_name"],
	  ...
	  $picname = $eidplace.'.jpg';
    //$imgdb = "img_url='$img_url',";
      $eppic = $picname;
	  ...
	  $image->load($path1);
      //$image->resizeToWidth(194);
     // $image->save($path1main);
...
//$elatitude = $_POST['latitude']; $elongitude

$result = mysql_query("INSERT INTO iplace (ecnumber,ecnumber2,elatitude,elongitude,ecity,eaddress,eemail,eppic,epname,elocation,estatus,eadminid,erank,edatereg,etype,esubtype,edetails) VALUES ('$ecnumber','$ecnumber2','$elatitude','$elongitude','$ecity','$eaddress','$eemail','$eppic','$epname','$ecountry','$estatus','$eadminid','$erank','$edatereg','$etype','$esubtype','$edetails')");

$upload_dir='';

/*if (!file_exists($upload_dir)) {
    mkdir($upload_dir, 0777);
	mkdir($upload_dir."/large/", 0777);
	mkdir($upload_dir."/small/", 0777);
   }
*/
$eppic = '';
$epname = '';
...
if($_GET['id']!=""){
	...
	$result = mysql_query("SELECT * FROM iplace WHERE eidplace = '$idz'");
	while($row = mysql_fetch_array($result)) {
	//$product_name = $row['product_name'];
	$eidplace = $row['eidplace'];
	$eppic = $row['eppic'];
	$epname = $row['epname'];
...
$imgdb = "img_url='$img_url',";}}else{$img_url=$_POST[oldfile];}*/
...
//$idzaray = explode("$", $_GET['id']);
$result = mysql_query("UPDATE iplace SET $imgdb elatitude='$elatitude',elongitude='$elongitude',eaddress='$eaddress',eemail='$eemail',ecnumber='$ecnumber',ecity='$ecity',etype='$etype',epname='$epname',elocation='$ecountry',estatus='$estatus',eadminid='$eadminid',erank='$erank',edatereg='$edatereg',esubtype='$esubtype',edetails='$edetails' WHERE eidplace = '$idz'");
...
<body >
<form name="form1" enctype="multipart/form-data" method="post"  action="createplaces.php?id=<?php echo $idz; ?>">
...
<table width="100%" border="0" style="margin-top:100px">
  ...
  <tr>
    <td bgcolor="">Address(Main-Office)</td>
    <td bgcolor=""><label>
...
var res = theloc.split(",");
var defaultLat = 10.328057; //<?php if($elatitude!=""){ echo $elatitude; }else{ echo 'res[0]'; } ?>;//37.973787;
var defaultLng = 123.881462; //<?php if($elongitude!=""){ echo $elongitude; }else{ echo 'res[1]'; } ?>; //23.722426;
...
myMarker.setMap(map);
    ...
    <script src="eediz.js" type="text/javascript"></script>
<script type="text/javascript">
bkLib.onDomLoaded(function() {
	//new nicEditor().panelInstance('area1');
	new nicEditor({uploadURI : 'upsy.php', fullPanel : true}).panelInstance('area2');
...
//var codedata='';
  ...
  <tr>
     <td bgcolor="">Description(optional)</td>
     <td bgcolor=""><label>
       <textarea cols="100" id="area2" style="background-color:#FFF" name="details"><?php 
$details = str_replace('\"', '"', $edetails);
